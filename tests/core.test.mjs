...
import { normalizeBrushSettings, readBrushSettings, saveBrushSettings, DEFAULT_BRUSH_SETTINGS } from '../src/core/brushSettings.ts';
...
test('history returns to initial state, truncates redo and bounds memory', () => {
  ...
  h.move(1); h.move(1); h.push('d');
  h.move(-1); h.move(-1); h.move(-1);
  assert.equal(h.current, 'a');
  assert.equal(h.canUndo, false);
});
...
test('settings sanitize corrupt or out-of-range saved values', () => {
  ...
  assert.deepEqual(normalizeBrushSettings({ color: '#AABBCC', width: 90, eraserWidth: -3 }), { color: '#aabbcc', width: 40, eraserWidth: 1 });
