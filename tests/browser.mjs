...
const layout=await page.evaluate(()=>({
  toolbarY:[...document.querySelectorAll('.tool-button')].map(item=>Math.round(item.getBoundingClientRect().y)),
  settingsX:Math.round(document.querySelector('.brush-settings').getBoundingClientRect().x)
}));
assert.equal(new Set(layout.toolbarY).size,1);
assert.ok(layout.settingsX <= 20);
await page.getByRole('button',{name:'10 像素',exact:true}).click();
await page.getByRole('button',{name:'选择颜色 #1971c2',exact:true}).click();
assert.equal(await page.getByRole('button',{name:'撤销',exact:true}).isDisabled(),true);
await draw(); await count(1);
assert.deepEqual(await board(b=>({color:b.getObjects()[0].stroke,width:b.getObjects()[0].strokeWidth})),{color:'#1971c2',width:10});
...
await page.getByRole('button',{name:'40 像素',exact:true}).click();
...
assert.deepEqual(await board(b=>b.getBrushSettings()),{color:'#1971c2',width:10,eraserWidth:40});
console.log('PASS separate eraser size and settings persistence');
await board(b=>b.getCanvas().upperCanvasEl.focus());
await page.keyboard.press('5');
assert.equal(await board(b=>b.getDrawingTool()),'arrow');
...
console.log('PASS tool shortcuts and restored objects remain erasable');
...
await page.getByRole('button',{name:'同步预览',exact:true}).click();
assert.equal(await page.locator('.mirror-panel').isVisible(),true);
await page.getByRole('button',{name:'关闭同步预览',exact:true}).click();
...
console.log('PASS Excalidraw-style layout, sync preview and page controls');
