...
模拟函数位于 `src/utils/previewSync.ts`，在发送与接收两处留有注释，可作为将来接入真实 WebSocket 的位置。`src/core/preview.ts` 管理预览画布的串行加载和清理。目前没有 WebSocket 服务、连接或消息接收代码；预览仅在同一个页面中模拟数据编码与恢复，不模拟网络延迟、断线或多人协作。仅在内容变更时更新，指针移动和橡皮擦拖影不会触发全量同步。
...
画布保持 800×450 的逻辑尺寸，在手机屏幕上随可用宽度等比例缩小，上下两块画布同步适配；工具栏在窄屏改为横向排列。页面本身不应横向溢出。目前只有一个页面入口，不需要 `views/` 目录。
